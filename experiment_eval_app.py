...
def show_visuals(df: pd.DataFrame, index_col: str):
    ...
    for col in cols:
        ...
        chart = (bars + text).properties(height=300, width={'step':80})
...
def main():
    ...
    # Existing metrics differences
    diff['conversion_rate'] = f"{int(round((totals_df.loc['Test','conversion_rate'] - totals_df.loc['Control','conversion_rate']) * 10000))} bps"
    ...
    styled = totals_with_diff.style.format(fmt_dict)
    ...
    primary = max(contributors, key=contributors.get) if net_sales_impact >= 0 else min(contributors, key=contributors.get)
    ...
    df_lo = df[df['order_status'].isin(['L', 'O'])]
    visitor_stats = df_lo.groupby(['buckets', 'exposed_visitor_id']).agg(total_sales=('net_sales', 'sum'), order_count=('order_id', 'nunique')).assign(net_aov=lambda x: x.total_sales / x.order_count, orders_per_converted=lambda x: x.order_count).reset_index()
    ...
    # Segment Impact Analysis (unchanged)
    # ... same as original ...
